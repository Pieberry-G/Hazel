...
namespace Hazel {
	...
	void EditorLayer3D::OnImGuiRender()
	{
		...
		ImVec2 minBound = { windowPos.x + viewportMinRegion.x, windowPos.y + viewportMinRegion.y };
		ImVec2 maxBound = { windowPos.x + viewportMaxRegion.x, windowPos.y + viewportMaxRegion.y };
	...
	void EditorLayer3D::OnEvent(Event& e)
	{
		m_EditorCamera.OnEvent(e);

		EventDispatcher dispatcher(e);
		dispatcher.Dispatch<KeyPressedEvent>(HZ_BIND_EVENT_FN(EditorLayer3D::OnKeyPressed));
		dispatcher.Dispatch<MouseButtonPressedEvent>(HZ_BIND_EVENT_FN(EditorLayer3D::OnMouseBottonPressed));
	}

	bool EditorLayer3D::OnKeyPressed(KeyPressedEvent& e)
	{
		// Shortcuts
		if (e.IsRepeat())
			return false;
	...
	bool EditorLayer3D::OnMouseBottonPressed(MouseButtonPressedEvent& e)
	{
	...
	void EditorLayer3D::OpenScene(const std::filesystem::path& path)
	{
		if (m_SceneState != SceneState::Edit)
			OnSceneStop();
	...
	void EditorLayer3D::SaveScene()
	{
		if (!m_EditorScenePath.empty())
			SerializeScene(m_ActiveScene, m_EditorScenePath);
	}

	void EditorLayer3D::SaveSceneAs()
	{
	...
	void EditorLayer3D::SerializeScene(Ref<Scene> scene, const std::filesystem::path& path)
	{
		SceneSerializer serializer(scene);
		serializer.Serialize(path.string());
	}

	void EditorLayer3D::OnScenePlay()
	{
	...
	void EditorLayer3D::OnSceneStop()
	{
	...
	void EditorLayer3D::OnDuplicateEntity()
	{
		if (m_SceneState != SceneState::Edit)
			return;
