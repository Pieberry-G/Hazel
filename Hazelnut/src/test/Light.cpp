...
namespace Hazel {
    ...
    void Light::createEnvGeometryHandler()
    {
        ...
        _envGeometryHandler->loadGeometry(_viewer->_searchPath.find(envSphere));
    }

    void Light::loadEnvironmentLight()
    {
        // Load the requested radiance map.
        mx::ImagePtr envRadianceMap = _viewer->_imageHandler->acquireImage(_envRadianceFilename);
        if (!envRadianceMap)
        {
        ...
        if (_normalizeEnvironment)
        {
            ...
            {
                _viewer->_imageHandler->saveImage("NormalizedRadiance.hdr", envRadianceMap);
            }
        }
        ...
        if (_splitDirectLight)
        {
            ...
            if (_saveGeneratedLights)
            {
                _viewer->_imageHandler->saveImage("IndirectRadiance.hdr", envRadianceMap);
        ...
        if (!_normalizeEnvironment && !_splitDirectLight)
        {
            ...
            envIrradianceMap = _viewer->_imageHandler->acquireImage(envIrradiancePath);
        }
        ...
        if (!envIrradianceMap || envIrradianceMap->getWidth() == 1)
        {
            if (_generateReferenceIrradiance)
            {
                ...
                if (_saveGeneratedLights)
                {
                    _viewer->_imageHandler->saveImage("ReferenceIrradiance.hdr", envIrradianceMap);
                }
            }
            else
            {
                ...
                if (_saveGeneratedLights)
                {
                    _viewer->_imageHandler->saveImage("SphericalHarmonicIrradiance.hdr", envIrradianceMap);
                }
            }
        }

        // Release any existing environment maps and store the new ones.
        _viewer->_imageHandler->releaseRenderResources(_lightHandler->getEnvRadianceMap());
        _viewer->_imageHandler->releaseRenderResources(_lightHandler->getEnvPrefilteredMap());
        _viewer->_imageHandler->releaseRenderResources(_lightHandler->getEnvIrradianceMap());
        ...
        if (!_splitDirectLight)
        {
            ...
            _lightRigFilename = _viewer->_searchPath.find(_lightRigFilename);
            if (_lightRigFilename.exists())
            {
                _lightRigDoc = mx::createDocument();
                mx::readFromXmlFile(_lightRigDoc, _lightRigFilename, _viewer->_searchPath);
            }
            else
            {
    ...
    void Light::splitDirectLight(mx::ImagePtr envRadianceMap, mx::ImagePtr& indirectMap, mx::DocumentPtr& dirLightDoc)
    {
        mx::Vector3 lightDir;
    ...
    mx::MaterialPtr Light::getEnvironmentMaterial()
    {
        if (!_envMaterial)
        {
            mx::FilePath envFilename = _viewer->_searchPath.find(mx::FilePath("resources/Lights/environment_map.mtlx"));
            try
            {
                _envMaterial = _viewer->_renderPipeline->createMaterial();
                _envMaterial->generateEnvironmentShader(_viewer->_genContext, envFilename, _viewer->_stdLib, _envRadianceFilename);
            }
