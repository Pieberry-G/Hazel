...
namespace Hazel {
    ...
    void Mesh::createGeometryHandler()
    {
        ...
        loadMesh(_viewer->_searchPath.find(_meshFilename));
    }

    void Mesh::loadMesh(const mx::FilePath& filename)
    {
        ...
        if (_geometryHandler->loadGeometry(filename))
        {
            ...
            {
                _viewer->_shadowMaterial->unbindGeometry();
            }

            _viewer->invalidateShadowMap();
        }
        else
        {
    ...
    void Mesh::applyDirectLights(mx::DocumentPtr doc)
    {
        if (_viewer->_light->getLightRigDoc())
        {
            doc->importLibrary(_viewer->_light->getLightRigDoc());
            _viewer->_xincludeFiles.insert(_viewer->_light->getLightRigFilename());
        }

        try
        {
            std::vector<mx::NodePtr> lights;
            _viewer->_light->getLightHandler()->findLights(doc, lights);
            _viewer->_light->getLightHandler()->registerLights(doc, lights, _viewer->_genContext);
            _viewer->_light->getLightHandler()->setLightSources(lights);
        }
    ...
    void Mesh::loadDocument(mx::DocumentPtr libraries)
    {
        ...
        _viewer->_genContext.clearUserData();
        ...
        try
        {
            ...
            mx::readFromXmlFile(doc, filename, _viewer->_searchPath, &readOptions);
            ...
            applyModifiers(doc, _viewer->_modifiers);
            ...
            for (size_t i = 0; i < renderablePaths.size(); i++)
            {
                ...
                if (udimSetValue && udimSetValue->isA<mx::StringVec>())
                {
                    ...
                    {
                        mx::MaterialPtr mat = _viewer->_renderPipeline->createMaterial();
                        mat->setDocument(doc);
            ...
            if (!newMaterials.empty())
            {
                ...
                _viewer->_imageHandler->setSearchPath(extendedSearchPath);
                ...
                for (mx::MaterialPtr mat : newMaterials)
                {
                    ...
                    _viewer->_genContext.clearNodeImplementations();
                    ...
                    if (!udim.empty())
                    {
                        ...
                        else
                        {
                            ...
                            mat->generateShader(_viewer->_genContext);
                            if (udimElement == elem)
                            {
                                udimMaterial = mat;
                            }
    ...
    mx::ImagePtr Mesh::getAmbientOcclusionImage(mx::MaterialPtr material)
    {
        const mx::string AO_FILENAME_SUFFIX = "_ao";
        const mx::string AO_FILENAME_EXTENSION = "png";

        if (!material || !_viewer->_genContext.getOptions().hwAmbientOcclusion)
        {
            return nullptr;
        }
        ...
        return _viewer->_imageHandler->acquireImage(aoFilename);
    }

    mx::MaterialPtr Mesh::getWireframeMaterial()
    {
        if (!_wireMaterial)
        {
            try
            {
                mx::ShaderPtr hwShader = mx::createConstantShader(_viewer->_genContext, _viewer->_stdLib, "__WIRE_SHADER__", mx::Color3(1.0f));
                _wireMaterial = _viewer->_renderPipeline->createMaterial();
    ...
    void Mesh::assignMaterial(mx::MeshPartitionPtr geometry, mx::MaterialPtr material)
    {
    ...
    mx::FilePath Mesh::getBaseOutputPath()
    {
        mx::FilePath baseFilename = _viewer->_searchPath.find(_materialFilename);
    ...
    void Mesh::updateMaterialSelections()
    {
