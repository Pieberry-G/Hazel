...
namespace Hazel {
    ...
    void GLRenderPipeline::updatePrefilteredMap()
    {
        ...
        try
        {
            mx::ShaderPtr hwShader = mx::createEnvPrefilterShader(genContext, RendererMX::s_Data->_stdLib, "__ENV_PREFILTER__");
        ...
        try
        {
            ...
            while (w > 0 && h > 0)
            {
                ...
                RendererMX::renderScreenSpaceQuad(material);
    ...
    mx::ImagePtr GLRenderPipeline::getShadowMap(int shadowMapSize)
    {
        ...
        if (!RendererMX::s_Data->_shadowMap)
        {
            ...
            if (!RendererMX::s_Data->_shadowMaterial)
            {
                try
                {
                    ...
                    RendererMX::s_Data->_shadowMaterial = mx::GlslMaterial::create();
                    RendererMX::s_Data->_shadowMaterial->generateShader(hwShader);
                }
                catch (std::exception& e)
                {
                    std::cerr << "Failed to generate shadow shader: " << e.what() << std::endl;
                    RendererMX::s_Data->_shadowMaterial = nullptr;
                }
            }
            if (!RendererMX::s_Data->_shadowBlurMaterial)
            {
                try
                {
                    mx::ShaderPtr hwShader = mx::createBlurShader(genContext, stdLib, "__SHADOW_BLUR_SHADER__", "gaussian", 1.0f);
                    RendererMX::s_Data->_shadowBlurMaterial = mx::GlslMaterial::create();
                    RendererMX::s_Data->_shadowBlurMaterial->generateShader(hwShader);
                }
                catch (std::exception& e)
                {
                    std::cerr << "Failed to generate shadow blur shader: " << e.what() << std::endl;
                    RendererMX::s_Data->_shadowBlurMaterial = nullptr;
                }
            }

            if (RendererMX::s_Data->_shadowMaterial && RendererMX::s_Data->_shadowBlurMaterial)
            {
                // Create framebuffer.
                ...
                RendererMX::s_Data->_shadowMaterial->bindShader();
                for (auto mesh : geometryHandler->getMeshes())
                {
                    RendererMX::s_Data->_shadowMaterial->bindMesh(mesh);
                    RendererMX::s_Data->_shadowMaterial->bindViewInformation(shadowCamera);
                    for (size_t i = 0; i < mesh->getPartitionCount(); i++)
                    {
                        mx::MeshPartitionPtr geom = mesh->getPartition(i);
                        RendererMX::s_Data->_shadowMaterial->drawPartition(geom);
                    }
                }
                RendererMX::s_Data->_shadowMap = framebuffer->getColorImage();
                ...
                for (unsigned int i = 0; i < RendererMX::s_Data->_shadowSoftness; i++)
                {
                    framebuffer->bind();
                    RendererMX::s_Data->_shadowBlurMaterial->bindShader();
                    if (imageHandler->bindImage(RendererMX::s_Data->_shadowMap, blurSamplingProperties))
                    {
                        mx::GLTextureHandlerPtr textureHandler = std::static_pointer_cast<mx::GLTextureHandler>(imageHandler);
                        int textureLocation = textureHandler->getBoundTextureLocation(RendererMX::s_Data->_shadowMap->getResourceId());
                        if (textureLocation >= 0)
                        {
                            std::static_pointer_cast<mx::GlslMaterial>(RendererMX::s_Data->_shadowBlurMaterial)
                                ->getProgram()->bindUniform("image_file", mx::Value::createValue(textureLocation));
                        }
                    }
                    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
                    RendererMX::renderScreenSpaceQuad(RendererMX::s_Data->_shadowBlurMaterial);
                    imageHandler->releaseRenderResources(RendererMX::s_Data->_shadowMap);
                    RendererMX::s_Data->_shadowMap = framebuffer->getColorImage();
                }
        ...
        return RendererMX::s_Data->_shadowMap;
    }
    ...
    void GLRenderPipeline::renderFrame(void*, int shadowMapSize, const char* dirLightNodeCat)
    {
        ...
        shadowState.ambientOcclusionGain = RendererMX::s_Data->_ambientOcclusionGain;
        ...
        if (RendererMX::s_Data->_drawEnvironment)
        {
            mx::MaterialPtr envMaterial = RendererMX::s_Data->_light->getEnvironmentMaterial();
            if (envMaterial)
            {
                const mx::MeshList& meshes = RendererMX::s_Data->_light->getEnvGeometryHandler()->getMeshes();
            ...
            {
                RendererMX::s_Data->_drawEnvironment = false;
            }
        }

        // Enable backface culling if requested.
        if (!RendererMX::s_Data->_renderDoubleSided)
        {
        ...
        // Transparent pass
        if (RendererMX::s_Data->_renderTransparency)
        {
            glEnable(GL_BLEND);
        ...
        if (RendererMX::s_Data->_outlineSelection)
        {
            ...
            {
                RendererMX::s_Data->_outlineSelection = false;
            }
        }
    }
